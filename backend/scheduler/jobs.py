...
refresh_all_watch_zones iterates over every zone in WATCH_ZONES once per
firing on an 8-hour cycle. Zones whose cached alert is still fresh (< 8 h
old) are skipped without hitting GDELT or Gemma. A 5-second sleep between
zones that trigger a live refresh prevents GDELT Cloud rate-limit errors.

The job is added to an AsyncIOScheduler in the FastAPI lifespan (main.py).
It reads shared connectors from app.state to avoid re-initialising per-job,
but constructs short-lived GDELT connectors per-call (they are lightweight
and their Redis reference is stable on app.state).
"""
...
async def refresh_all_watch_zones(app) -> None:  # noqa: ANN001
    """Refresh alerts for all WATCH_ZONES, skipping zones that are still fresh."""
    ...
    for zone in zones:
        ...
        if cached is not None:
            logger.info(f"scheduler: {region!r} days=1 still fresh — skipping")
            continue
